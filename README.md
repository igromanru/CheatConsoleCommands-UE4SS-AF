...
Free Crafting (Debug function) | freecraft \| freecrafting \| crafting \| craft |  | Allows player to craft ALL recipes, upgrade the Crafting Bench, instantly build furniture and unlock chests without keys. (Warning: You may need to restart the game to deactivate it completely!) (host only)
...
Build Anywhere | BuildAnywhere |  | Allows you to build deployables even if they overlap with other objects (Attention! The command might conflict with other build-related mods) (works as guest)
...
Krasue Cooldown | krasuecd \| rasuecooldown \| cdrasue | {minutes} | Changes Krasue's spawn cooldown in minutes (Default: 10min). The cooldown will be reapplied by the mod automatically each time you start the game. (To disable the command set value to 0 or 10) (host only)
Trap Krasue | trapkrasue \| containkrasue |  | Trap's Krasue in the next possible Containment Unit. (host only)
Free Krasue | freekrasue |  | Free Krasue from a Containment Unit. (host only)
Infinite Trait Points | inftraits \| inftrait \| traitpoints  \| inftraitpoints |  | Lets you select as many Traits as you want while creating a new character. (host only)
